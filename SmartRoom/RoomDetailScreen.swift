...
class RoomDetailViewModel: ObservableObject {
    ...
    private func loadRoomData() async {
        do {
            ...
            print("⚠️ API unavailable, showing fake data for room \(roomId): \(error.localizedDescription)")
            ...
            // Show fake data instead of error
            showFakeRoomData()
        }
    }
    
    // Show fake room data when API is not available
    private func showFakeRoomData() {
        // Set room name from constructor
        uiState.roomName = roomName
        
        // Fake temperature (already set above)
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [self] in
            // Fake lights based on room ID
            uiState.lights = getFakeLightsForRoom()
            uiState.isLoadingLights = false
        }
        ...
        {
            "status": 200,
            "message": "Success (Fake Data)",
            "data": {
                "content": [
                    {
                        "id": 1,
                        "name": "Đèn chính",
                        "description": "Đèn LED trần chính",
                        "isActive": true,
                        "level": 80,
                        "roomId": \(roomId)
                    },
        ...
        """
        uiState.apiResponse = fakeResponse
    }
    
    // Get fake lights for room
    private func getFakeLightsForRoom() -> [Light] {
        let fakeLights = [
            Light(id: roomId * 10 + 1, name: "Đèn chính", roomId: roomId, isActive: true, level: 80, description: "Đèn LED trần chính"),
            Light(id: roomId * 10 + 2, name: "Đèn bàn", roomId: roomId, isActive: false, level: 0, description: "Đèn bàn học"),
            Light(id: roomId * 10 + 3, name: "Đèn góc", roomId: roomId, isActive: true, level: 60, description: "Đèn góc phòng")
        ]
        return fakeLights
    }
    ...
    private func loadMockRoomData() {
        ...
        // Mock temperature based on room type
        uiState.currentTemp = getMockTemperatureForRoom()
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [self] in
            // Mock lights based on room ID and type
            uiState.lights = getMockLightsForRoom()
            uiState.isLoadingLights = false
        }
        
        // Mock API response
        do {
            let lightsData = try JSONEncoder().encode(uiState.lights)
            let lightsJson = String(data: lightsData, encoding: .utf8) ?? "[]"
            let mockResponse = """
            {
                "status": 200,
                ...
                "data": {
                    ...
                    "page": 0,
                    "size": 20,
                    "totalElements": \(uiState.lights.count),
                    "totalPages": 1
                },
                "timestamp": "\(ISO8601DateFormatter().string(from: Date()))"
            }
            """
            uiState.apiResponse = mockResponse
        } catch {
            uiState.apiResponse = "Error generating mock response: \(error.localizedDescription)"
        }
    }
    
    private func getMockTemperatureForRoom() -> Double {
        // Hardcode temperature to 26 degrees for all rooms
        return 26.0
    }
    
    private func getMockLightsForRoom() -> [Light] {
        let baseId = roomId * 10
        
        switch roomName.lowercased() {
        case let name where name.contains("bếp") || name.contains("kitchen"):
            return [
                Light(id: baseId + 1, name: "Đèn chính", roomId: roomId, isActive: true, level: 80, description: "Main kitchen light"),
                Light(id: baseId + 2, name: "Đèn bàn ăn", roomId: roomId, isActive: false, level: 0, description: "Dining table light"),
                Light(id: baseId + 3, name: "Đèn tủ bếp", roomId: roomId, isActive: true, level: 60, description: "Cabinet light")
            ]
        case let name where name.contains("khách") || name.contains("living"):
            return [
                Light(id: baseId + 1, name: "Đèn trần", roomId: roomId, isActive: true, level: 70, description: "Ceiling light"),
                Light(id: baseId + 2, name: "Đèn góc", roomId: roomId, isActive: false, level: 0, description: "Corner lamp"),
                Light(id: baseId + 3, name: "Đèn TV", roomId: roomId, isActive: true, level: 30, description: "TV ambient light")
            ]
        case let name where name.contains("ngủ") || name.contains("bed"):
            return [
                Light(id: baseId + 1, name: "Đèn chính", roomId: roomId, isActive: false, level: 0, description: "Main bedroom light"),
                Light(id: baseId + 2, name: "Đèn ngủ", roomId: roomId, isActive: true, level: 20, description: "Bedside lamp")
            ]
        case let name where name.contains("làm việc") || name.contains("office"):
            return [
                Light(id: baseId + 1, name: "Đèn bàn làm việc", roomId: roomId, isActive: true, level: 90, description: "Desk light"),
                Light(id: baseId + 2, name: "Đèn trần", roomId: roomId, isActive: true, level: 60, description: "Ceiling light")
            ]
        case let name where name.contains("gym") || name.contains("tập"):
            return [
                Light(id: baseId + 1, name: "Đèn LED thể thao", roomId: roomId, isActive: true, level: 100, description: "Sport LED light"),
                Light(id: baseId + 2, name: "Đèn gương", roomId: roomId, isActive: false, level: 0, description: "Mirror light")
            ]
        default:
            return [
                Light(id: baseId + 1, name: "Đèn chính", roomId: roomId, isActive: true, level: 50, description: "Main light"),
                Light(id: baseId + 2, name: "Đèn phụ", roomId: roomId, isActive: false, level: 0, description: "Secondary light")
            ]
        }
    }
